...
- Use the library before writing markup: `Button`/`IconButton` for actions, `Card` for panels, `Badge`/`StatusBadge`/`StatusDot` for state, `Input`/`Form*`/`Dropdown`/`Toggle` for forms, `Tabs`+`TabList`+`TabTrigger`+`TabContent` for tabs, `PageToolbar` for a list page's filter bar, `ScoreCard`/`ScoreCardGrid` for metrics, `Modal`/`Popup`/`FormPopup`/`ConfirmPopup` for dialogs, `Avatar`/`AvatarGroup` for agents and people.
