#!/bin/bash
# Tests for docs-read — run with: bash execute.test.sh
...
class H(BaseHTTPRequestHandler):
    def _reply(self, method, data=None):
        ...
        self.wfile.write(json.dumps({'success': True, 'data': {'id': 'd1', 'title': 'T', 'webViewLink': 'https://l', 'text': '# T body'}}).encode())
...
OUT=$(run --id d1)
check "flags: output" "$OUT" '{"id":"d1","title":"T","webViewLink":"https://l","text":"# T body"}'
check "flags: path" "$(jq -r .path "$STUB_LOG")" "/api/google/docs/d1"
run --id "https://docs.google.com/document/d/abc123/edit#x" >/dev/null
check "url: id extracted" "$(jq -r .path "$STUB_LOG")" "/api/google/docs/abc123"
run '{"documentId":"j1"}' >/dev/null
check "json: path" "$(jq -r .path "$STUB_LOG")" "/api/google/docs/j1"

echo "docs-read: $PASS passed, $FAIL failed"
[ "$FAIL" -eq 0 ]
