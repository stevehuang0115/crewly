#!/bin/bash
# Tests for drive-search — run with: bash execute.test.sh
...
class H(BaseHTTPRequestHandler):
    def _reply(self, method, data=None):
        ...
        self.wfile.write(json.dumps({'success': True, 'data': {'count': 1, 'files': [{'id': '1', 'name': 'Q3', 'mimeType': 'application/vnd.google-apps.document'}]}}).encode())
...
OUT=$(run --query "Q3 plan" --mime sheet --folder f1 --max 5)
check "flags: output" "$OUT" '{"count":1,"files":[{"id":"1","name":"Q3","mimeType":"application/vnd.google-apps.document"}]}'
check "flags: path" "$(jq -r .path "$STUB_LOG")" "/api/google/drive/files?q=Q3%20plan&mimeType=application%2Fvnd.google-apps.spreadsheet&folderId=f1&max=5"
run '{"query":"x"}' >/dev/null
check "json: path" "$(jq -r .path "$STUB_LOG")" "/api/google/drive/files?q=x"
run >/dev/null
check "no args: bare list" "$(jq -r .path "$STUB_LOG")" "/api/google/drive/files"
OUT=$(run --query no-grant || true)
check "not connected" "$(printf '%s' "$OUT" | jq -r .reason)" "not_connected"

echo "drive-search: $PASS passed, $FAIL failed"
[ "$FAIL" -eq 0 ]
