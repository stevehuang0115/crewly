#!/bin/bash
# Tests for slides-create — run with: bash execute.test.sh
...
class H(BaseHTTPRequestHandler):
    def _reply(self, method, data=None):
        ...
        self.wfile.write(json.dumps({'success': True, 'data': {'id': 'p9', 'title': data.get('title'), 'slideCount': len(data.get('slides', [])), 'webViewLink': 'https://l'}}).encode())
...
OUT=$(run --title "Pitch" --slides '[{"title":"Why","bullets":["a"]},{"title":"Plan"}]')
check "json slides: output" "$OUT" '{"success":true,"id":"p9","title":"Pitch","slideCount":2,"webViewLink":"https://l"}'
check "json slides: body" "$(jq -c .body "$STUB_LOG")" '{"title":"Pitch","slides":[{"title":"Why","bullets":["a"]},{"title":"Plan"}]}'
TMP="$(mktemp).md"; printf '# Highlights\n- Revenue +12%%\n* Two launches\n\n## Risks\nHiring is slow\n' > "$TMP"
run --title "Q3" --outline-file "$TMP" >/dev/null
check "outline: parsed" "$(jq -c .body.slides "$STUB_LOG")" '[{"title":"Highlights","bullets":["Revenue +12%","Two launches"]},{"title":"Risks","bullets":["Hiring is slow"]}]'
rm -f "$TMP"
OUT=$(run_err --title "x" --slides '[]' || true)
check "empty slides rejected" "$(printf '%s' "$OUT" | grep -c 'non-empty')" "1"

echo "slides-create: $PASS passed, $FAIL failed"
[ "$FAIL" -eq 0 ]
