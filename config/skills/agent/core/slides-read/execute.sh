#!/bin/bash
# =============================================================================
# slides-read — Read a Google Slides deck as text, slide by slide
#
# Backed by GET /api/google/slides/:id.
#
# Usage:
#   bash execute.sh --id <presentationId>
#   bash execute.sh '{"id":"<presentationId>"}'
# =============================================================================
set -euo pipefail
...
if [ -n "$INPUT_JSON" ]; then
  ...
  [ -z "$ID" ] && ID=$(printf '%s' "$INPUT" | jq -r '.id // .presentationId // empty')
fi
ID=$(printf '%s' "$ID" | sed -E 's#.*/presentation/d/([^/?]+).*#\1#')
require_param "id (--id)" "$ID"
RESPONSE=$(call GET "/google/slides/$(uri "$ID")") || { printf '%s\n' "$RESPONSE"; exit 1; }
if printf '%s' "$RESPONSE" | jq -e '.truncated == true' >/dev/null 2>&1; then printf '%s\n' "$RESPONSE"; exit 0; fi
printf '%s' "$RESPONSE" | jq -c '{id: .data.id, title: .data.title, slideCount: .data.slideCount, webViewLink: .data.webViewLink, slides: [.data.slides[] | {index, lines} + (if .notes then {notes} else {} end)]}'
