#!/bin/bash
# Tests for slides-read — run with: bash execute.test.sh
...
class H(BaseHTTPRequestHandler):
    def _reply(self, method, data=None):
        ...
        self.wfile.write(json.dumps({'success': True, 'data': {'id': 'p1', 'title': 'Pitch', 'slideCount': 1, 'webViewLink': 'https://l', 'slides': [{'index': 1, 'objectId': 'o', 'lines': ['Why'], 'notes': 'n'}]}}).encode())
...
OUT=$(run --id p1)
check "flags: output" "$OUT" '{"id":"p1","title":"Pitch","slideCount":1,"webViewLink":"https://l","slides":[{"index":1,"lines":["Why"],"notes":"n"}]}'
check "flags: path" "$(jq -r .path "$STUB_LOG")" "/api/google/slides/p1"
run --id "https://docs.google.com/presentation/d/pp/edit" >/dev/null
check "url: id extracted" "$(jq -r .path "$STUB_LOG")" "/api/google/slides/pp"

echo "slides-read: $PASS passed, $FAIL failed"
[ "$FAIL" -eq 0 ]
