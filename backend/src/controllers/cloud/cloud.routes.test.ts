...
jest.mock('./cloud.controller.js', () => ({
  connectToCloud: jest.fn((_req: any, res: any) => res.status(200).json({ success: true })),
  disconnectFromCloud: jest.fn((_req: any, res: any) => res.status(200).json({ success: true })),
  validateCloudToken: jest.fn((_req: any, res: any) => res.status(200).json({ success: true })),
  refreshCloudToken: jest.fn((_req: any, res: any) => res.status(200).json({ success: true })),
  getCloudStatus: jest.fn((_req: any, res: any) => res.status(200).json({ success: true })),
  getCloudTemplates: jest.fn((_req: any, res: any) => res.status(200).json({ success: true })),
  sendCloudMessage: jest.fn((_req: any, res: any) => res.status(200).json({ success: true })),
  getDeviceId: jest.fn((_req: any, res: any) => res.status(200).json({ success: true })),
}));

jest.mock('./relay.controller.js', () => ({
  getDevicesFromSync: jest.fn((_req: any, res: any) => res.status(200).json({ success: true })),
}));

jest.mock('./cloud-google-auth.controller.js', () => ({
  cloudGoogleStart: jest.fn((_req: any, res: any) => res.redirect('https://accounts.google.com')),
  cloudGoogleCallback: jest.fn((_req: any, res: any) => res.redirect('https://crewlyai.com')),
  cloudGoogleUrl: jest.fn((_req: any, res: any) => res.status(200).json({ success: true, data: { url: 'https://accounts.google.com' } })),
  cloudGoogleCallbackPost: jest.fn((_req: any, res: any) => res.status(200).json({ success: true, data: {} })),
}));
