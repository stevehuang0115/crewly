...
| **6** ✅ **部分完成 2026-09-20** | 触达 | 见 §6.6 | G7（部分） | 复用 REST-over-relay | 实际 1 天 |
...
**通电（2026-09-20 补完）**：三个模块已接进运行时，见 §6.7。
...
### 6.6 第 6 期已交付内容（2026-09-20）

**没有新造传输层。** 手机 App 用的 REST-over-relay 已经在跑且已鉴权，把桌面路由加进白名单就够了——再搭一条 WebSocket 通道只会多一处要维护、要防护的东西。

`/api/desktop/*`（形状刻意对齐 `/api/browser/*`）：

| 路由 | 作用 | 经 relay |
|---|---|---|
| `GET /status` | 权限就绪？有人在用？暂停了？停止了？ | ✅ |
| `POST /look` | snapshot / ocr / screenshot / displays / resolve / wait-for | ✅ |
| `POST /stop` | 从任何地方叫停（`{resume:true}` 解除） | ✅ |
| `POST /act` | 动鼠标键盘 | ❌ **故意不开** |

**这个不对称是本期最重要的决定。** 看一台机器在干什么、以及能把它停下来，正是不在电脑前的 owner 需要的，而且两者都不会造成伤害。但 `/act` 动的是一台真 Mac 上的真键盘——不能因为手机 App 能够到这里的其他东西，它就顺带能够到内网里一台电脑的鼠标。测试里专门有一条断言锁死这点。

路由层自己也有动作白名单（读 9 个 / 写 11 个，两组互斥且有测试保证不会悄悄长到一起），未知动作会**列出可用的 20 个**而不是只说「不支持」。所有路由都 shell 到同一个 skill，护栏因此仍然只有一个执行点——而且是**唯一一个能从互联网够到的**，这点尤其重要。

**`request-human`** —— 被我删掉的 `vnc-browser` 空壳本来想做的事，诚实版本比 VNC 隧道小得多，也覆盖了真正会发生的情况：CAPTCHA、2FA、登录、需要 owner 判断。agent 暂停自己、说明卡在哪、**存一张截图**（39KB，owner 在手机上就能看），然后等。用暂停而不是停止——任务没被取消。它**在暂停状态下仍然可用**，否则 agent 唯一的选择就是重试那件它已经做不到的事。

**平台守卫**：非 macOS 上现在明确返回 `unsupported_platform` 并说明去用浏览器工具或 API skill。之前是 `osascript: command not found`——读起来像装坏了，于是 agent 会重试。

**测试**：38 项，0 失败 0 跳过。

第三次撞上同一个 `set -o pipefail` 陷阱（`cmd | grep -q` 在 cmd 正确地非零退出时整条判失败）之后，我加了 `assert_output` 助手把这个错误从测试里根除，而不是再逐处修补。

**明确未做**（G7 只完成一部分）：
- **Linux / Windows 后端**。现在是诚实拒绝，不是支持。Linux 无头（Xvfb + AT-SPI + ydotool）是 steamfun-ops 那类机器的真实需求，工作量接近重写一遍感知层和动作层。
- **noVNC 真人远程接管**。`request-human` 覆盖了「需要人做一件事」，但覆盖不了「owner 想亲手操作这台远程机器」。
- **审计回放 UI**（第 5 期同样欠着）：缩略图已成对落盘，但没有页面可翻。

### 6.7 任务层通电（2026-09-20）

第 4 期做完时，那三个模块是「测试齐全但没人调用」——对一件安全机制来说这是最没用的状态。现在接上了。

**`desktop_task` 工具**（plan / step / confirm / summary / abandon）。三件事让它**难以绕过**，而不只是「可用」：

1. **`step` 是唯一的前进方式。** agent 不能把子目标标记为完成，它只能问，由检查点回答。
2. **未验证的工作会让整个 turn 变成 incomplete。** 这条最关键：turn 本身可能结束得好好的——模型觉得自己做完了才停的。而**那恰恰是要抓的情况**：一个「相信自己保存了文件并如实汇报」的 agent，从外面看和真的保存了的 agent 一模一样。新增 `IncompleteReason: 'desktop-unverified'`，复用今天早些时候为 DeepSeek 做的那套 incomplete 机制，owner 会收到「部分桌面任务无法验证，可能并没有真的发生」。
3. **不可逆动作走既有审批队列**，owner 在他本来就处理审批的地方回答。批准 → 继续；拒绝 → 明确禁止找别的路子。

两个小设计：`plan` 时**一次性给出每个子目标的选路建议**——等 agent 已经打开应用再说「其实有个 skill」就晚了；`confirm` 遇到可逆动作会**明说「不用问」**，因为一个什么都问的 agent 学不到边界在哪。
